...
public class UserService {
    ...
    public StatusDto signup(UserRequestDto userRequestDto) {
        if (userRequestDto.getUsername() == null || userRequestDto.getPassword() == null) {
            return new StatusDto(StatusEnum.FAIL.name(), "아이디, 비밀번호를 입력하세요");
        }

        if (!Pattern.matches("^[a-zA-Z0-9]{3,}$", userRequestDto.getUsername())) {
            return new StatusDto(StatusEnum.FAIL.name(), "아이디는 영어, 숫자만 써야하고 3~20글자 만 가능합니다.");
        }

        if (!userRequestDto.getPassword().equals(userRequestDto.getPasswordCheck())) {
            //비밀번호, 비밀번호확인 다름
            return new StatusDto(StatusEnum.FAIL.name(),"비밀번호를 확인해 주세요");
        }

        if (userRequestDto.getPassword().contains(userRequestDto.getUsername())) {
            return new StatusDto(StatusEnum.FAIL.name(), "비밀번호에는 닉네임과 같은 값이 포함될 수 없습니다.");
        }
        if (userRequestDto.getPassword().length() < 4) {
            return new StatusDto(StatusEnum.FAIL.name(), "비밀번호는 4글자 이상 입력해 주세요.");
        }
        User user = userRepository.findByUsername(userRequestDto.getUsername());
        if (user != null) {
            return new StatusDto(StatusEnum.FAIL.name(), "이미 존재하는 아이디 입니다.");
        }
        userRequestDto.setPassword(passwordEncoder.encode(userRequestDto.getPassword()));
        userRepository.save(new User(userRequestDto));

        return new StatusDto(StatusEnum.SUCCESS.name(), "회원가입 성공");
    }
